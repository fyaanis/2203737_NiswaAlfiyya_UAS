...
func main() {
	InitDB()
	defer DB.Close()
	log.Println("Server Running on Port 9080")

	// Router
	router := mux.NewRouter()
	router.HandleFunc("/inventory_niswaalfiyya", GetInventoryItems).Methods("GET")
	router.HandleFunc("/inventory_niswaalfiyya", CreateInventoryItem).Methods("POST")
	router.HandleFunc("/inventory_niswaalfiyya/{id}", GetInventoryItem).Methods("GET")
	router.HandleFunc("/inventory_niswaalfiyya/{id}", UpdateInventoryItem).Methods("PUT")
	router.HandleFunc("/inventory_niswaalfiyya/{id}", DeleteInventoryItem).Methods("DELETE")
	router.HandleFunc("/inventory_niswaalfiyya/search", SearchInventoryByName).Methods("GET")

	// Add CORS middleware
	handler := &CORSRouterDecorator{router}

	// Start server with error handling
	err := http.ListenAndServe(":9080", handler)
	if err != nil {
		log.Fatal(err)
	}
}
...
func GetInventoryItems(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_niswaalfiyya")
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func CreateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	stmt, err := DB.Prepare("INSERT INTO inventory_niswaalfiyya(nama_barang, jumlah, harga_satuan, lokasi, deskripsi) VALUES(?,?,?,?,?)")
	if err != nil {
		panic(err.Error())
	}
...
func GetInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_niswaalfiyya WHERE id = ?", params["id"])
	if err != nil {
		panic(err.Error())
	}
	defer result.Close()
...
func UpdateInventoryItem(w http.ResponseWriter, r *http.Request) {
	w.Header().Set("Content-Type", "application/json")
	params := mux.Vars(r)
	stmt, err := DB.Prepare("UPDATE inventory_niswaalfiyya SET nama_barang = ?," +
...
func DeleteInventoryItem(w http.ResponseWriter, r *http.Request) {
	...
	stmt, err := DB.Prepare("DELETE FROM inventory_niswaalfiyya WHERE id = ?")
	if err != nil {
		panic(err.Error())
	}
...
func SearchInventoryByName(w http.ResponseWriter, r *http.Request) {
	...
	result, err := DB.Query("SELECT id, nama_barang, jumlah, harga_satuan, lokasi, deskripsi FROM inventory_niswaalfiyya WHERE nama_barang LIKE ?", "%"+searchTerm+"%")
	if err != nil {
...
func InitDB() {
	DB, err = sql.Open("mysql", "root:@/db_2203737_niswaalfiyya_uas")
	if err != nil {
		panic(err)
	}
